...
static int fts_cmdpkg_wirte(u8 ctrl, u8 *cmd, u32 cmdlen)
{
	...
	txbuf = kzalloc(FTX_MAX_COMMMAND_LENGTH + SPI_HEADER_LENGTH, GFP_KERNEL);
	if (NULL == txbuf) {
		FTS_ERROR("txbuf kalloc fail");
		ret = -ENOMEM;
	}

	txbuf[pos++] = COMMAND_PACKAGE;
	txbuf[pos++] = ctrl | (cmdlen & 0x0F);
	for (i = 0; i < cmdlen; i++) {
		txbuf[pos++] = cmd[i];
	}

	ret = fts_spi_transfer(txbuf, NULL, pos);
	kfree(txbuf);
	return ret;
}

int fts_boot_write(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
	int ret = 0;
	u8 *txbuf = NULL;
	u32 txlen = 0;
